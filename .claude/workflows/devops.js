export const meta = {
  name: "devops",
  description: "devops の成果物 DAG を 1 巡させる（役 13 ／ 機械 5 ／ 人間ゲート 3）",
  whenToUse: "成果物定義 devops.mk に沿って、要求から受入検査まで1 パス通したいとき。args.root に作業根を渡す",
  phases: [
    { title: "第1段", detail: "要求" },
    { title: "第2段", detail: "探針結果" },
    { title: "第3段", detail: "仕様" },
    { title: "第4段", detail: "受入検査仕様 / 実装物_v(n) / 計器" },
    { title: "第5段", detail: "変異試験の合否 / 実行可能物 / 整合したドキュメント / 設計保証 / 非回帰の合否" },
    { title: "第6段", detail: "ゼロ点の実測" },
    { title: "第7段", detail: "受入検査報告書" },
    { title: "第8段", detail: "バグ票 / 観測項目" },
    { title: "第9段", detail: "リポジトリの状態 / 残留リスク報告 / 解除判定" },
  ],
}

// 自動生成。手で編集しない。正は /Users/shinichi/work/PairAgenticClaude/models/devops.mk
// 生成: python tools/emit_claude.py /Users/shinichi/work/PairAgenticClaude/models/devops.mk
//
...
const NODES = {
  "要求": {
    ...
    "task": "elicit_intent",
    "by": "agent",
    "agentType": "devops-elicit-intent",
    "gate": "H1",
    "path": "upstream/要求.md",
    "deps": [
      {
  ...
  "探針結果": {
    ...
    "task": "run_probe",
    "by": "agent",
    "agentType": "devops-run-probe",
    "gate": "",
    "path": "upstream/探針結果.md",
    "deps": [
      {
        "name": "要求",
        "node": "要求",
  ...
  "仕様": {
    ...
    "by": "agent",
    "agentType": "devops-write-specification",
    "gate": "H2",
    "path": "upstream/仕様.md",
    "deps": [
      {
    ...
    "post": [
      "不足なし ／ 過剰なし",
      "各項目が判定手続きに落とせる",
      "探針結果が「原理的にできない」と言った項目を、そのまま残していない",
  ...
  "計器": {
    ...
    "by": "agent",
    "agentType": "devops-build-instrument",
    "gate": "",
    "path": "instruments/計器.py",
    "deps": [
      {
        "name": "仕様",
        "node": "仕様",
  ...
  "ゼロ点の実測": {
    ...
    "by": "machine",
    "agentType": "devops-measure-baseline",
    "gate": "",
    "path": "checks/ゼロ点の実測.md",
    "deps": [
      {
        "name": "計器",
        "node": "計器",
  ...
  "受入検査仕様": {
    ...
    "by": "agent",
    "agentType": "devops-design-acceptance-tests",
    "gate": "",
  ...
  "実装物": {
    ...
    "post": [
      "仕様を満たす",
      "ホワイトボックステストを含む ／ 縮退経路にログがある"
    ],
    "out": [
      "実行可能なコード ／ ホワイトボックステスト ／ 縮退経路のログ",
      "理由 ＋ 残留リスク"
    ],
    "skills": [
      "ホワイトボックステストの書き方",
  ...
  "設計保証": {
    ...
    "task": "assure_design",
    "by": "agent",
    "agentType": "devops-assure-design",
    "gate": "",
    "path": "upstream/設計保証.md",
    "deps": [
      {
        "name": "実装物_v(n)",
        "node": "実装物",
  ...
  "受入検査報告書": {
    ...
    "by": "agent",
    "agentType": "devops-execute-acceptance-tests",
    "gate": "H3",
    ...
    "out": [
      "判定（合格／不合格／**保留**）",
      "差分 ／ 実施条件（**実行が環境に何を残したか**を含む）／ カバーできなかった領域",
      "計器で読んだ量と、ゼロ点からの差",
      "残留リスク"
    ],
    "skills": [
  ...
  "観測項目": {
    ...
    "by": "agent",
    "agentType": "devops-register-observation",
    "gate": "",
    "path": "telemetry/観測項目.md",
    "deps": [
      {
        "name": "受入検査報告書",
        "node": "受入検査報告書",
    ...
    "post": [
      "解除条件が書かれている（「〜になったら何をするか」が無い待ちを作らない）",
      "解除条件が実データで判定できる形になっている"
    ],
    "out": [
      "観測する項目 ＋ **解除条件** ＋ 実施日"
    ],
  ...
  "解除判定": {
    ...
    "by": "agent",
    "agentType": "devops-evaluate-release-condition",
    "gate": "",
    "path": "telemetry/解除判定.md",
    "deps": [
      {
        "name": "観測項目",
        "node": "観測項目",
    ...
    "post": [
      "過去に記録された観測値ではなく、実データを読み直して判定している",
      "日付ではなくデータで判定している"
    ],
    "out": [
  ...
  "バグ票": {
    ...
    "post": [
      "赤なら起票している（自分の変更由来かを問わない）",
      "診断・解き方を「事実」として書いていない（症状と実測だけ）"
    ],
    "out": [
      "症状 ／ 実測 ／ 証拠 ／ タイムライン"
    ],
  ...
  "整合したドキュメント": {
    ...
    "by": "agent",
    "agentType": "devops-reconcile-documents",
    "gate": "",
    "path": "docs/整合したドキュメント.md",
    "deps": [
      {
        "name": "実装物_v(n)",
        "node": "実装物",
  ...
  "リポジトリの状態": {
    ...
    "by": "agent",
    "agentType": "devops-sync-repository",
    "gate": "",
    "path": "リポジトリの状態",
    "deps": [
      {
  ...
  "変異試験の合否": {
    ...
    "by": "machine",
    "agentType": "devops-run-mutation-testing",
    "gate": "",
  ...
  "非回帰の合否": {
    ...
    "task": "run_regression",
    "by": "machine",
    "agentType": "devops-run-regression",
    "gate": "",
    "path": "checks/非回帰の合否.md",
    "deps": [
      {
  ...
  "残留リスク報告": {
    ...
    "by": "machine",
    "agentType": "devops-aggregate-residual-risk",
    "gate": "",
...
const HUMAN = [
  {
    ...
    "post": [
      "受け手はこれだけを見て修正に着手できる",
      "受け手が読めない語彙を含まない",
      "**どのゲートで差し戻したかが書かれている**（H1/H2/H2b/H3 のどれか）"
    ],
    "out": [
      "差し戻し先（暫定）／ なぜ差し戻されるか（受け手の語彙で）／ 再発回数"
    ]
  }
]
